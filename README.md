...
| Posts | `Done post`: what a finished agent posts, the screen tail (default), its last reply from the Claude Code or Codex transcript, or that reply rendered with bold, lists, links and code (transcript folders: [Where transcripts are read](docs/behaviour.md#where-transcripts-are-read); phone pushes outside Telegram: [Ringing the phone through Bark](docs/behaviour.md#ringing-the-phone-through-bark)); `Turn summary line` (on): `⏱ 4 min · fable-5-1 · ✏️ 3 files · ↑ 12k tokens · 🧠 126k` under every done post, from the Claude Code transcript; `Fold long replies after` (20 lines): a long reply arrives collapsed behind an arrow, the summary line stays visible; `React to prompts` (off): 👀 / 👌 on your message once the agent took it and when the turn ends; `Questions in the bot's chat` (on): a question is posted silently into the topic and rings from the bot's private chat with a link; `Question delay`: wait up to 120 s for a second capture and stay silent when the question was answered in Herdr meanwhile; `Skip short done posts`: no done post for a turn shorter than N seconds; `Done screen lines` (12): how many terminal lines a done screen post carries; `Answer Telegram prompts when seen` (off): a prompt sent from Telegram is answered there even when the turn ended in the tab you were watching; `Send pictures from the reply` (on): a screenshot or other picture the agent's reply names, written in the last day, follows the done post as a photo, once per topic ([Pictures from the reply](docs/behaviour.md#pictures-from-the-reply)); `Trim the input frame` (on): Claude Code's input box, status line and mode hint are cut from the bottom of every screen post |
...
| `Telegram Agents: send test message` | Posts a test message into General straight from the action (the daemon need not run), rings a test push through Bark when `bark_url` is set, and reports the outcome |
