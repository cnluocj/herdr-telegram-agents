...
func TestPanelOpenShowsGroupsAndRetiresPrevious(t *testing.T) {
	...
	if got := texts(sent[0].Buttons); strings.Join(got, "|") != "Sync|Quiet|Posts|Inbox|Appearance|Privacy|Topics|✖ Close" {
...
func TestPanelPostsGroup(t *testing.T) {
	...
	if got := texts(f.tg.Buttons(900)); strings.Join(got, "|") != "Screen Done post|☑ Turn summary line|20 lines Fold long replies after|☑ Trim the input frame|☐ React to prompts|☑ Questions in the bot's chat|Off Question delay|Off Skip short done posts|12 lines Done screen lines|☐ Answer Telegram prompts when seen|☑ Send pictures from the reply|↺ Reset to defaults|‹ Back|✖ Close" {
