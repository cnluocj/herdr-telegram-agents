...
func TestMappingStoreMigratesV1WithoutLosingTopicState(t *testing.T) {
	...
	if !ok || m.Version != 1 || m.ChatID != -1001 || m.Dashboard != 777 ||
		...
		!e.Closed || !e.Muted || e.Cwd != "" || e.AgentKind != "" ||
	...
	if !strings.Contains(string(raw), `"version": 2`) || strings.Contains(string(raw), `"cwd"`) || strings.Contains(string(raw), `"agent_kind"`) {
	...
	if !ok || got.Version != domain.MappingVersion || got.Dashboard != 777 || e.ThreadID != 42 || e.Status != domain.StatusExited || !e.Closed || !e.Muted || e.Cwd != "" || e.AgentKind != "" {
